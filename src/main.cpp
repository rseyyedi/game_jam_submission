...
// configuration step. It creates a namespace called `game_jam_submission`. You can modify
...
template<std::size_t Width, std::size_t Height> struct GameBoard
{
  ...
  std::string set_label(std::size_t x, std::size_t y) { return fmt::format("{:^4}", strings.at(x).at(y)); }
...
int main(int argc, const char **argv)
{
 ...
 Options:
    ...
    std::map<std::string, docopt::value> args = docopt::docopt(USAGE,
      ...
      fmt::format("{} {}",
        game_jam_submission::cmake::project_name,
        game_jam_submission::cmake::project_version));// version string, acquired
